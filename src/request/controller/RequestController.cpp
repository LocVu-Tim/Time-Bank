...
vector<userRequest *> RequestController::filterBasedOnHostRating(vector<userRequest *> &requestList,double hostRating) {
  ...
  for (int i = 0; i < requestList.size(); i++) {
    // if the request is not for host, skip
    if (requestList[i]->minimumRatingForHost == -1) {
      continue;
    }
    // skip if the request is for supporter
    else if (requestList[i]->minimumRatingForSupporter != -1) {
	  continue;
	}
    // if the user < minimum rating, skip
    else if (this->user->getHostRating() < requestList[i]->minimumRatingForHost) {
      continue;
    }
    // if not available, skip
    else if (requestList[i]->availability == false) {
      continue;
    }
    // if the supporterId is the current user, skip
    else if (requestList[i]->hostId == this->user->getUserId()) {
      continue;
    }
  ...
  return filteredList;
    ...
    void RequestController::viewAllRequests()
    {
        ...
        while (choice)
        {
            ...
            requestView.viewAllRequests(availableData, this->userList);
            cin >> input;
            if (input == "Y" || input == "y")
            {
    ...
    vector<userRequest *> RequestController::filterBasedOnSupporterRating(vector<userRequest *> & requestList, double supporterRating)
    {
      vector<userRequest *> filteredList;
      for (int i = 0; i < requestList.size(); i++) {
        // if the request is not for supporter, skip
        /*if (requestList[i]->minimumRatingForSupporter == -1) {
          continue;
        }*/
        // skip if the request is for host
        if (requestList[i]->minimumRatingForHost != -1) {
		  continue;
		}
        // if the user < minimum rating, skip
        if (this->user->getSupporterRating() < requestList[i]->minimumRatingForSupporter) {
          continue;
        }
        // if not available, skip
        if (requestList[i]->availability == false) {
          continue;
        }
        // if the supporterId is the current user, skip
        if (requestList[i]->supporterId == this->user->getUserId()) {
          continue;
        }
        filteredList.push_back(requestList[i]);
      }
