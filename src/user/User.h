...
public:
  ...
  User(string userName, string pwd, string fullName, string email,
       string homeAddr, string phoneNo, vector<int> blocked,
       vector<string> skills, int creds, int role, int userId,
       double skillRatingScore, double supporterRatingScore,
       double hostRatingScore, vector<Rating> ratings);
  ...
  friend bool caseInsensitiveStringCompare(const string &str1,
                                           const string &str2);

  // method to compare strings (case-sensitive)
  friend bool caseSensitiveStringCompare(const string &str1,
                                         const string &str2);
  ...
  friend User *findByUsername(const vector<User *> &users,
  ...
  friend User registerMember(vector<User *> users);
};
